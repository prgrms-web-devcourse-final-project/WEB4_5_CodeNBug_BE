...
public class AdminService {
    ...
    /**
     * 여러 사용자 계정을 일괄 비활성화합니다.
     * 
     * @param userIds 비활성화할 사용자 ID 목록
     * @return 비활성화된 계정 수
     */
    @Transactional
    public int batchDisableAccounts(List<Long> userIds) {
        log.info(">> 일괄 계정 비활성화 서비스 호출: {}개 계정", userIds.size());
        
        if (userIds == null || userIds.isEmpty()) {
            log.warn(">> 비활성화할 계정이 없습니다.");
            return 0;
        }
        ...
        for (Long userId : userIds) {
            try {
                ...
                if (user == null) {
                    log.warn(">> 계정 비활성화 실패: 사용자를 찾을 수 없음 - ID={}", userId);
                    continue;
                }
                
                // 이미 비활성화된 계정은 건너뜀
                if (!user.isEnabled()) {
                    log.info(">> 이미 비활성화된 계정: ID={}", userId);
                    continue;
                }
                ...
                log.info(">> 계정 비활성화 완료: ID={}, 이메일={}", userId, user.getEmail());
                disabledCount++;
                
            } catch (Exception e) {
                log.error(">> 계정 비활성화 중 오류 발생: ID={}, 오류={}", userId, e.getMessage(), e);
            }
        }
        
        log.info(">> 일괄 계정 비활성화 완료: 성공={}/{}", disabledCount, userIds.size());
        return disabledCount;
    }

    /**
     * 여러 사용자 계정을 일괄 활성화합니다.
     * 
     * @param userIds 활성화할 사용자 ID 목록
     * @return 활성화된 계정 수
     */
    @Transactional
    public int batchEnableAccounts(List<Long> userIds) {
        log.info(">> 일괄 계정 활성화 서비스 호출: {}개 계정", userIds.size());
        
        if (userIds == null || userIds.isEmpty()) {
            log.warn(">> 활성화할 계정이 없습니다.");
            return 0;
        }
        ...
        for (Long userId : userIds) {
            try {
                ...
                if (user == null) {
                    log.warn(">> 계정 활성화 실패: 사용자를 찾을 수 없음 - ID={}", userId);
                    continue;
                }
                
                // 이미 활성화된 계정은 건너뜀
                if (user.isEnabled()) {
                    log.info(">> 이미 활성화된 계정: ID={}", userId);
                    continue;
                }
                ...
                log.info(">> 계정 활성화 완료: ID={}, 이메일={}", userId, user.getEmail());
                enabledCount++;
                
            } catch (Exception e) {
                log.error(">> 계정 활성화 중 오류 발생: ID={}, 오류={}", userId, e.getMessage(), e);
            }
        }
        
        log.info(">> 일괄 계정 활성화 완료: 성공={}/{}", enabledCount, userIds.size());
        return enabledCount;
    }

    /**
     * 여러 사용자 계정을 일괄 잠금 처리합니다.
     * 
     * @param userIds 잠금할 사용자 ID 목록
     * @return 잠금 처리된 계정 수
     */
    @Transactional
    public int batchLockAccounts(List<Long> userIds) {
        log.info(">> 일괄 계정 잠금 서비스 호출: {}개 계정", userIds.size());
        
        if (userIds == null || userIds.isEmpty()) {
            log.warn(">> 잠금 처리할 계정이 없습니다.");
            return 0;
        }
        ...
        for (Long userId : userIds) {
            try {
                ...
                if (user == null) {
                    log.warn(">> 계정 잠금 처리 실패: 사용자를 찾을 수 없음 - ID={}", userId);
                    continue;
                }
                
                // 이미 잠금 상태인 계정은 건너뜀
                if (user.isAccountLocked()) {
                    log.info(">> 이미 잠금 상태인 계정: ID={}", userId);
                    continue;
                }
                ...
                // Redis에 계정 잠금 상태 설정
                String email = user.getEmail();
                loginAttemptService.lockAccount(email);
                
                log.info(">> 계정 잠금 처리 완료: ID={}, 이메일={}", userId, email);
                lockedCount++;
                
            } catch (Exception e) {
                log.error(">> 계정 잠금 처리 중 오류 발생: ID={}, 오류={}", userId, e.getMessage(), e);
            }
        }
        
        log.info(">> 일괄 계정 잠금 처리 완료: 성공={}/{}", lockedCount, userIds.size());
        return lockedCount;
    }

    /**
     * 여러 사용자 계정의 잠금을 일괄 해제합니다.
     * 
     * @param userIds 잠금 해제할 사용자 ID 목록
     * @return 잠금 해제된 계정 수
     */
    @Transactional
    public int batchUnlockAccounts(List<Long> userIds) {
        log.info(">> 일괄 계정 잠금 해제 서비스 호출: {}개 계정", userIds.size());
        
        if (userIds == null || userIds.isEmpty()) {
            log.warn(">> 잠금 해제할 계정이 없습니다.");
            return 0;
        }
        ...
        for (Long userId : userIds) {
            try {
                ...
                if (user == null) {
                    log.warn(">> 계정 잠금 해제 실패: 사용자를 찾을 수 없음 - ID={}", userId);
                    continue;
                }
                
                // 잠금 상태가 아닌 계정은 건너뜀
                if (!user.isAccountLocked()) {
                    log.info(">> 이미 잠금 해제된 계정: ID={}", userId);
                    continue;
                }
                ...
                log.info(">> 계정 잠금 해제 완료: ID={}, 이메일={}", userId, email);
                unlockedCount++;
                
            } catch (Exception e) {
                log.error(">> 계정 잠금 해제 중 오류 발생: ID={}, 오류={}", userId, e.getMessage(), e);
            }
        }
        
        log.info(">> 일괄 계정 잠금 해제 완료: 성공={}/{}", unlockedCount, userIds.size());
