package org.codenbug.user.global.security.aspect;
...
public class RoleRequiredAspect {
	...
	public Object checkRole(ProceedingJoinPoint joinPoint, RoleRequired roleRequired) throws Throwable {
		...
		log.debug("Checking roles: required={}, method={}", Arrays.toString(requiredRoles), joinPoint.getSignature());

		boolean hasRequiredRole = Arrays.stream(requiredRoles)
			.anyMatch(role -> authentication.getAuthorities().contains(new SimpleGrantedAuthority(role.getRole())));

		if (!hasRequiredRole) {
			log.warn("Access denied: user={}, required roles={}, method={}", authentication.getName(),
		...
		log.debug("Access granted: user={}, method={}", authentication.getName(), joinPoint.getSignature());
